...
class eventController {
  static async addEvent(req, res, next) {
    try {
      ...
      let expectedPrice = `${formatPrice(
        courtPrice / limitParticipants
  ...
  static async joinEvent(req, res, next) {
    try {
      ...
      let newExpectedPrice = `${formatPrice(
        currentEvent.courtPrice / newTotalParticipants
      )} - ${formatPrice(currentEvent.courtPrice)}`;
  ...
  static async leaveEvent(req, res, next) {
    try {
      ...
      let newExpectedPrice = `${formatPrice(
        event.courtPrice / newTotalParticipant
      )} - ${formatPrice(event.courtPrice)}`;
